      ...
      <div className="space-y-6 mb-8">
        {wpComments.map(comment => (
          <div key={`wp-${comment.id}`} className="bg-card-bg p-4 border border-border rounded">
            ...
            <div className="text-sm text-accent prose prose-sm max-w-none prose-p:text-accent prose-strong:text-accent prose-li:text-accent prose-a:text-highlight" dangerouslySetInnerHTML={{ __html: comment.content.rendered }} />
          </div>
        ))}
